...
func (h *SampleHelper) GetWorkflow(ctx context.Context, workflowID string, value interface{}) error {
	workflowClient, err := h.Builder.BuildCadenceClient()
	if err != nil {
		h.Logger.Error("Failed to build cadence client.", zap.Error(err))
		return err
	...
	err = workflowClient.GetWorkflow(context.Background(), workflowID, "").Get(ctx, value)
	if err != nil {
		h.Logger.Error("Failed to cancel workflow", zap.Error(err))
		return err
	}
...
func (h *SampleHelper) registerWorkflowAndActivity(worker worker.Worker) {
	...
	for _, act := range h.activityRegistries {
		if len(act.alias) == 0 {
			worker.RegisterActivityWithOptions(act.registry, activity.RegisterOptions{
